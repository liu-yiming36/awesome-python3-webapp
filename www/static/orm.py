...
# 建立一个异步io的连接池
async def create_pool(loop, **kw):  # 检查过了
    # 打印日志, **kw为字典
...
# 封装select函数
async def select(sql, args, size=None):  # 检查过了
    log(sql, args)
...
async def execute(sql, args, autocommit=True):    # 检查过了
    log(sql)  # 日志记录
    async with __pool.get() as conn:  # 打开连接池
        if not autocommit:  # 如果autocommit为False, conn.begin()开始事务
            await conn.begin()
        try:  # 执行sql语句
            # 用async with conn.cursor as cur:该句报错
            async with conn.cursor(aiomysql.DictCursor) as cur:
                await cur.execute(sql.replace('?', '%s'), args)
                affected = cur.rowcount  # 通过rowcount获得sql语句操作影响的行数，此处误加入了cur.rowcount()报错，不能加括号
            if not autocommit:  # 如果autocommit为False, conn.begin()开始事务
                await conn.commit()  # 此处误写为conn.begin()
        except BaseException as e:  # 处理出错情况
            if not autocommit:
                await conn.rollback()  # 情况不对就回滚,类似电脑重启包治百病  # 此处rollback之前写错成roolback
            raise
        return affected
...
def create_args_string(num):  # 检查了
    L = []
    for n in range(num):
        L.append('?')
    return ', '.join(L)
...
class StringField(Field):
    ...
    # 谨记此处是ddl是三个字母,不是数字1.由于这个原因找bug找了半天
...
class ModelMetaclass(type):   # 检查过了

    def __new__(cls, name, bases, attrs):  # 四份参数依次为：当前准备创建的类的对象, 类的名字, 类继承的父类集合, 类的方法集合
        # 排除对Model类的修改,如没有该if会导致在attrs里无任何id, name, password, email,导致报错
        if name == 'Model':
            return type.__new__(cls, name, bases, attrs)
        ...
        escaped_fields = list(map(lambda f: '`%s`' % f, fields))  # repr语法代入SQL语句不会报错
...
class Model(dict, metaclass=ModelMetaclass):  # 检查了
    ...
    async def findNumber(cls, selectField, where=None, args=None):
        ...
        sql = ['select %s _num_ from `%s`' % (selectField, cls.__table__)]  # __table__类属性：存储表名字, _num_不知道有啥用
        if where:
            sql.append('where')
            sql.append(where)
    ...
    async def update(self):
        ...
        args.append(self.getValue(self.__primary_key__))  # 此处误写为appenf
